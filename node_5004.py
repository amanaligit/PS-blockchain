...
class BlockChain:
    ...
    def add_manu(self, productinfo, num, process):
        self.transactions.append({'productinfo': productinfo,
                                'num': num,
                                'process': process,
                                })
...
@app.route('/mine_manu', methods=['GET'])
def mine_manu():
    blockchain.replace_chain()
    previous_block = blockchain.get_previous_block()
    previous_hash = blockchain.hash(previous_block)
    mined_block = blockchain.proof_of_work(previous_hash, "manufacturing")
    blockchain.create_block(mined_block)
    response = {'message': 'Congratulations, you just mined a block!',
                'index': mined_block['index'],
                'timestamp': mined_block['timestamp'],
...
def index():
    ...
    return render_template("manu.html", message="Please enter the Manufacturing details to continue")
...
@app.route('/add_manu', methods=['POST'])
def add_manu():
    ...
    if len(process)==0 or productinfo == "":
        return render_template("error.html", message="Please fill all the details")
    index = blockchain.add_manu(productinfo, num, process)
    response = f'This transaction will be added to block {index}'
    return render_template("manu.html", message=response)
